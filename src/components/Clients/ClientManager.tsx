      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-card p-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Répertoire et historique de facturation
          </p>
        </div>
        ...
        <button
          ...
          className="hover-press app-btn-primary w-full sm:w-auto px-4 py-3 sm:py-2.5 font-semibold flex items-center justify-center gap-2 text-sm cursor-pointer"
        >
      ...
      <div className="flex flex-col gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="glass-input w-full pl-11 pr-4 py-3 text-slate-800 text-sm"
          />
        </div>

        <div className="glass-segment overflow-x-auto max-w-full">
          <button
            type="button"
            onClick={() => setCategoryFilter('all')}
            className={`glass-segment-btn ${categoryFilter === 'all' ? 'is-active' : ''}`}
          >
            Tous ({clients.length})
          </button>
          <button
            ...
            className={`glass-segment-btn inline-flex items-center gap-1.5 ${categoryFilter === 'entreprise' ? 'is-active' : ''}`}
          >
          ...
          <button
            ...
            className={`glass-segment-btn inline-flex items-center gap-1.5 ${categoryFilter === 'personne_physique' ? 'is-active' : ''}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredClients.map((client) => {
            ...
            return (
              <div
                key={client.id}
                className="hover-lift glass-card p-5 flex flex-col justify-between"
              >
                <div>
                  {/* Category Badge & Actions Header */}
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider bg-brand-glow/15 text-brand-glow border-brand-glow/30">
                      {isEntreprise ? <Building className="w-3 h-3" /> : <User className="w-3 h-3" />}
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        title="Modifier"
                        className="p-1.5 text-slate-300 hover:text-brand-sand hover:bg-white/10 rounded-lg transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-300 hover:text-rose-300 hover:bg-rose-500/15 rounded-lg transition-colors cursor-pointer"
                      >
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 bg-brand-glow/15 text-brand-glow border border-brand-glow/25">
                      {isEntreprise && client.companyName
                        ? client.companyName.charAt(0)
                        : client.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-bold text-brand-sand text-base leading-snug truncate">
                        {isEntreprise && client.companyName ? client.companyName : client.name}
                      </h3>
                      {isEntreprise && client.companyName && (
                        <p className="text-xs text-slate-300 font-medium truncate">Contact: {client.name}</p>
                      )}
                    </div>
                  </div>

                  <div className="space-y-1.5 text-xs text-slate-200 mb-4 bg-white/5 p-3 rounded-xl border border-white/10">
                    {client.email && (
                      <div className="flex items-center gap-2">
                        <Mail className="w-3.5 h-3.5 text-brand-glow shrink-0" />
                        <span className="truncate">{client.email}</span>
                      </div>
                    )}
                    {client.phone && (
                      <div className="flex items-center gap-2">
                        <Phone className="w-3.5 h-3.5 text-brand-glow shrink-0" />
                        <span>{client.phone}</span>
                      </div>
                    )}
                    {client.address && (
                      <div className="flex items-center gap-2">
                        <MapPin className="w-3.5 h-3.5 text-brand-glow shrink-0" />
                    ...
                    {client.nifRccm && (
                      <div className="flex items-center gap-2 font-mono text-[11px] text-slate-300 pt-1 border-t border-white/10 mt-1">
                        <Building className="w-3.5 h-3.5 text-brand-glow shrink-0" />
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
                    <div className="bg-brand-glow/10 border border-brand-glow/25 p-2.5 rounded-xl">
                      <div className="text-slate-300 text-[10px] uppercase tracking-wider font-semibold">
                        Total Réglé
                      </div>
                      <div className="kpi-figure font-bold text-brand-sand text-sm mt-0.5">
                    ...
                    <div className="bg-white/5 border border-white/10 p-2.5 rounded-xl">
                      ...
                      <div className="font-bold text-brand-sand text-sm mt-0.5">
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-white/10">
                  <button
                    ...
                    className="flex-1 py-1.5 px-2 bg-brand-glow/15 hover:bg-brand-glow/25 text-brand-glow border border-brand-glow/30 font-semibold rounded-lg text-xs transition-colors text-center cursor-pointer flex items-center justify-center gap-1"
                  >
                    <span>+ Facture</span>
                  </button>
                  <button
                    ...
                    className="flex-1 py-1.5 px-2 bg-white/8 hover:bg-white/14 text-brand-sand border border-white/15 font-semibold rounded-lg text-xs transition-colors text-center cursor-pointer flex items-center justify-center gap-1"
                  >
                    <span>+ Devis</span>
                  </button>
                </div>
